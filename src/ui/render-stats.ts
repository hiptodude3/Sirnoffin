...
export function renderStats(char: Character, onBack: () => void): void {
  renderHeader(char);
  ...
  const rows = KEYS.map(k =>
    `<div class="stat-row"><span class="stat-label">${k.toUpperCase()}</span>`
    + `<span class="stat-value">${s[k]}</span>`
    + `<button class="btn btn-sm" data-stat="${k}"${
      char.statPoints < 1 ? ' disabled' : ''}>+</button></div>`
  ).join('');

  main.innerHTML =
    `<h2 style="color:#7a7aff">Stats</h2>`
    + `<p>Points: <strong>${char.statPoints}</strong></p>`
